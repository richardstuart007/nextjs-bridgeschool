...
/* --------------------------------------------------------------------*/
async function create_users_idx(client) {
  try {
    await client.sql`CREATE EXTENSION IF NOT EXISTS "uuid-ossp"`

    const create_Table = await client.sql`
CREATE UNIQUE INDEX IF NOT EXISTS users_user
...
`
    ...
    console.error('Error seeding users_user:', error)
    throw error
  }
}
/* --------------------------------------------------------------------*/
